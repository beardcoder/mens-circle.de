import { LatLngExpression, map, icon, marker, tileLayer } from 'leaflet';
import './Map.css';
import { createComponent } from "../../../Assets/utils/createComponent.ts";

createComponent('[data-component="event-location"]', ({ element }) => {
  const latLong: LatLngExpression = [
    parseFloat(element.dataset.lat!),
    parseFloat(element.dataset.long!),
  ];
  ...
  marker(latLong, { icon: markerIcon, interactive: false })
    .addTo(locationMap)
