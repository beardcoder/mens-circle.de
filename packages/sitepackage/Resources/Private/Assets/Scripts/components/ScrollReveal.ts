...
export const createScrollReveal = (): ScrollRevealFactory => {
  ...
  const revealAll = (): void => {
    const allElements = [
      ...Array.from(elements.text),
      ...Array.from(elements.cards),
    ];
  ...
  // Public API
  const reveal = (newElements: NodeListOf<Element> | Element[]): void => {
    if (!observer) return;

    const elementsArray = Array.isArray(newElements)
      ? newElements
      : Array.from(newElements);
